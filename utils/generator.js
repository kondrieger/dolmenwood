...
import { dice as DICE } from './dice.js'
...
function buildProfile(opts) {
  var kin = D.KINDREDS[opts.kindred];
  if (opts.mode === 'kindredclass') {
    ...
    return {
      ...
      armourEn: kc.armourEn, weaponsEn: kc.weaponsEn,
      ...
      glamoursByLevel: kc.glamoursByLevel || null,
  ...
  return {
    ...
    armourEn: c.armourEn, weaponsEn: c.weaponsEn,
    weaponWhitelist: c.weaponWhitelist || null,
    noMissile: !!c.noMissile,
    ...
    glamoursByLevel: c.glamoursByLevel || null,
...
function classAllowedFor(kindredId, classId) {
  var kin = D.KINDREDS[kindredId];
  var c = D.CLASSES[classId];
...
function generate(opts) {
  ...
  for (var li = 0; li < extraLangCount; li++) {
    ...
    while (tries < 30) {
      var lr = DICE.roll(1, 6);
      var found = null;
      for (var q = 0; q < D.COMMON_LANGUAGES.length; q++) {
        var L = D.COMMON_LANGUAGES[q];
  ...
  if (glamourCount > 0) {
    ...
    for (var gi = 0; gi < glamourCount; gi++) {
      ...
      while (gtries < 40) {
        var gr = DICE.roll(1, 20);
        var cand = D.GLAMOURS[gr.total - 1];
  ...
  if (prof.grantsLesserRune) {
    var rr = DICE.roll(1, 6);
    var rune = D.LESSER_RUNES[rr.total - 1];
  ...
  if (prof.grantsSpellBook) {
    var br = DICE.roll(1, 6);
    var book = D.SPELL_BOOKS[br.total - 1];
  ...
  if (prof.magicType === 'holy') {
    ...
    if (r1count > 0) {
      ...
      for (var hi = 0; hi < r1count; hi++) {
        var hr = DICE.roll(1, 8);
        var hs = D.HOLY_R1[hr.total - 1];
  ...
  ch.portraitPrompt = portraitPrompt ? portraitPrompt(ch) : null;
...
function generateEquipment(log, kin, prof, opts) {
  ...
  /* Общие предметы */
  D.STARTING_GENERAL.equipped.forEach(function (id) { equipped.push(gearItem(id)); });
  D.STARTING_GENERAL.backpack.forEach(function (id) { stowed.push(gearItem(id)); });
  var backpack = gearItem('backpack');

  /* Броня */
  var armour = null, shield = false;
  if (prof.startArmour) {
    var ar = DICE.roll(1, 6);
    var arow = null;
    ...
    var armDesc = picked.length ? picked.map(function (id) {
      return id === 'shield' ? D.SHIELD.ru : D.ARMOUR[id].ru;
    }).join(' + ') : 'без брони';
  ...
  for (var w = 0; w < 2; w++) {
    ...
    items.forEach(function (id) {
      if (D.WEAPONS[id]) { var it = weaponItem(id); weapons.push(it); names.push(it.ru); }
      else if (D.GEAR[id]) { var g = gearItem(id); stowed.push(g); names.push(g.ru); }
    });
  ...
  (prof.classItems || []).forEach(function (id) {
    if (id === 'instrument_any') {
      var ir = DICE.roll(1, 2);
      var chosen = ir.total === 1 ? 'instrument_string' : 'windinstrument';
      log.record('equipment', 'Музыкальный инструмент (струнный/духовой)', ir, { result: D.GEAR[chosen].ru });
      dice.push({ sides: 6, results: ir.dice, label: 'инструмент', value: ir.total });
      equipped.push(gearItem(chosen));
      lines.push('Предмет класса: ' + D.GEAR[chosen].ru);
      return;
    }
    var g = gearItem(id);
  ...
  for (var a = 0; a < 4; a++) {
    ...
    while (tries < 30) {
      var adr = DICE.roll(1, 20);
      var cand = D.ADVENTURING_ITEMS[adr.total - 1];
      var dup = advItems.indexOf(cand.id) >= 0;
      log.record('equipment', 'Приключенческий предмет ' + (a + 1) + ' (d20)', adr,
        { result: D.GEAR[cand.id].ru + (dup ? ' — уже есть, перебрасываем' : '') });
  ...
  lines.push('Приключенческие предметы: ' + advItems.map(function (id) { return D.GEAR[id].ru; }).join(', '));
  ...
  return {
    ...
    equipment: {
      armour: armour,
      shield: shield ? { id: 'shield', ru: D.SHIELD.ru, en: D.SHIELD.en, acBonus: 1, slots: 1, weight: 100, cost: 10 } : null,
      weapons: weapons,
      equipped: equipped,
      stowed: stowed,
      container: backpack,
...
function countsForLoad(item) {
  ...
  if (D.LOAD_EXEMPT_IDS.indexOf(item.id) >= 0) return false;
  if (D.LOAD_EXEMPT_CATS.indexOf(item.cat) >= 0) return false;
  return true;
}
...
function computeLoad(ch) {
  ...
  return { total: w, max: D.MAX_LOAD, parts: parts };
}
...
function computeSpeed(ch) {
  ...
  var s = countSlots(ch);
  var speed = 10;
  for (var i = 0; i < D.WEIGHT_ENCUMBRANCE.length; i++) {
    if (load.total <= D.WEIGHT_ENCUMBRANCE[i].maxWeight) { speed = D.WEIGHT_ENCUMBRANCE[i].speed; break; }
  }
  return {
    value: speed,
    load: load.total,
